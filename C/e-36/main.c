...
int main(int argc, char **argv){
	...
	if(stat(f1, &st1) == -1){
		errx(2, "error while stat %s", f1);
	}
	
	if(stat(f2, &st2) == -1){
		errx(3, "error while stat %s", f2);
	}
	
	if(stat(f3, &st3) == -1){
		errx(4, "error while stat %s", f3);
	}

	if(!(st1.st_mode & S_IREAD) || !(st2.st_mode & S_IREAD)){
	...
	if((st1.st_size % sizeof(fmt) !=0) || (st2.st_size % sizeof(uint32_t) !=0)){
		errx(6, "one or both files %s and %s is/are corrpted",f1, f2);
	}
	...
	fd3 = open(f3, O_CREAT | O_TRUNC | O_WRONLY, S_IREAD | S_IWRITE);
	if(fd3 == -1){
		const int olderrno = errno;
		clo(fd1, fd2, fd3);
		errno = olderrno;
